...
> Answer: Client-side routing loads all data from the pages in memory and will display them when that page is being accessed. This allows for better performance after initial load of the pages, and prevents client from needing to send a frequent request to the server. Overall, can provide a better user experience.
